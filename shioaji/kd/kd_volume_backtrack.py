...
sql = "SELECT * FROM `semi_kd_volume_determine` ORDER BY `date` DESC LIMIT 30"

mycursor.execute(sql)
myresult = mycursor.fetchall()

verify_date = ['1','3','5','10','20','30']
...
for data in myresult:
    ...
    conflict_time_modify = str(conflict_time.days)
    ...
    if conflict_time_modify == '1' :
        sql = "INSERT INTO `semi_kd_volume_backtrack`(`date`, `original_prediction`, `1day_result`, `3day_result`, `5day_result`, `10day_result`, `20day_result`, `30day_result`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
        val = str(date), orginal_prediction, str(verify_result), "", "", "", "", ""
    elif conflict_time_modify == '3':
        sql = 'UPDATE `semi_kd_backtrack` SET `3day_result` = "'+str(verify_result)+'" WHERE `date` = "' + str(date)+'"'
    ...
    if conflict_time_modify == '1':
        ...
        mydb.commit()
    elif conflict_time_modify == '3' or conflict_time_modify == '5' or conflict_time_modify == '10' or conflict_time_modify == '20' or conflict_time_modify == '30':
        mycursor.execute(sql)
        mydb.commit()
