...
for data in stonk_list:
    sql = "SELECT `k`, `d`, `volume` FROM `"+str(data)+"` ORDER BY `date` DESC LIMIT 8"
    ...
    today_volume = int(myresult[0][2])

    for item in myresult:
        k_list.append(item[0])
        d_list.append(item[1])

    pass_volume = int(myresult[1][2]) + int(myresult[2][2])+ int(myresult[3][2])+ int(myresult[4][2])+ int(myresult[5][2])+ int(myresult[6][2])+ int(myresult[7][2])

    pass_avg_volume = pass_volume / 7

    if float(d_list[1]) > float(k_list[1]):  #如果昨天的d值大於昨天k值
        if float(k_list[0]) > float(d_list[0]): #如果今天k值大於今天d值
            print(str(data) + "黃金交叉")

            if today_volume / pass_avg_volume > 6:
                print("量體倍數:" + str(round(today_volume/pass_avg_volume, 2)))
                print("量爆增")
                cross.append(str(data))
                cross_status.append("1")
            elif today_volume / pass_avg_volume > 4:
                print("量體倍數:" + str(round(today_volume/pass_avg_volume, 2)))
                print("量增")
                cross.append(str(data))
                cross_status.append("1")
            elif today_volume / pass_avg_volume > 2:
                print("量體倍數:" + str(round(today_volume/pass_avg_volume, 2)))
                print("量微增")
                cross.append(str(data))
                cross_status.append("1")
            elif today_volume / pass_avg_volume < 1:
                print("量體倍數:" + str(round(today_volume/pass_avg_volume, 2)))
                print("量縮")
                '''
                cross.append(str(data))
...
sql = "INSERT INTO `semi_kd_volume_determine`(`date`, `stock_num`, `status`, `volume`, `volume_status`) VALUES (%s, %s, %s, %s, %s)"
