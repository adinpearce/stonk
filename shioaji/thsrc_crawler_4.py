...
second_anchor = first_anchor[2]
data = (second_anchor.text).split('\n')
new_data = []
for item in data:
    if item == "度/月份" or item == "列車開行次數" or item == "列車行駛里程(公里)" or item == "座位公里數(座公里)":
        pass
    elif  item == "":
        pass
    else:
...
for total in new_data:
    if len(new_data) != 0:
        ...
        sql = "INSERT INTO `thsrc_3`(`date`, `train_depart_total`, `train_ride_km`, `seat_km`) VALUES (%s, %s, %s, %s)"
